...
$(document).on('click', 'a#check_for_updates', function(e){
	...
	var api_url = isMobileDevice() ? 'https://api.unfoldingword.org/obs/txt/1/obs-catalog.json' : 'http://unfoldingword.jacobwiley.com/app/api.php?callback=?';
	    ...
	    $.getJSON(api_url, api_data, function(json){
	    	...
	    	for(var i = 0; i < json.length; i++){
	    		...
	    		if(update_language){
					...
					var api_url = isMobileDevice() ? 'https://api.unfoldingword.org/obs/txt/1/'+lang+'/obs-'+lang+'.json' : 'http://unfoldingword.jacobwiley.com/app/api.php?callback=?';
